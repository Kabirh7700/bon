...
import { Kpi, Order, FilterOptions, Supplier, ProductType, ManagedUser, UserRole } from '../types.ts';
import { PackageIcon, DollarSignIcon, AlertTriangleIcon, GlobeAltIcon, ArchiveBoxIcon, BanIcon, WalletIcon, ReceiptPercentIcon, CreditCardIcon } from '../components/icons/DashboardIcons.tsx';
...
const calculateKPIs = (
  ...
  let totalDelayedOrdersInView = 0; 
  ...
  nonCancelledOrdersInView.forEach(order => {
    totalClientValue += order.totalFinalPrice; 
    totalItemsSold += order.totalQuantity;
    order.clientPayments.forEach(p => totalAmountCollected += p.amountPaid);

    if (order.paymentStatus === 'Overdue') overdueClientPaymentsCount++;

    let isOrderDelayed = false;
    for (const historyItem of order.stageHistory) {
        if (historyItem.isDelayed && historyItem.stage !== OrderStatus.PAID && historyItem.stage !== OrderStatus.CANCELLED) {
            isOrderDelayed = true;
            break;
        }
    }
    if (isOrderDelayed) totalDelayedOrdersInView++;
  ...
  return [
    ...
    { title: 'Total Items Sold', value: totalItemsSold, unit: '(Non-Cancelled in View)', icon: <ArchiveBoxIcon className="w-6 h-6"/>, status: 'good' },
    { title: 'Total Client Value', value: `$${totalClientValue.toLocaleString()}`, unit:'(Non-Cancelled in View)', icon: <DollarSignIcon className="w-6 h-6"/>, status: 'good' },
    { title: 'Total Collected (Client)', value: `$${totalAmountCollected.toLocaleString()}`, unit:'(Non-Cancelled in View)', icon: <WalletIcon className="w-6 h-6"/>, status: 'good' }, 
    {
      title: 'Delayed Orders',
      value: overallDelayedOrdersCount, 
      unit: `(${(totalNonCancelledOrdersOverall > 0 ? (overallDelayedOrdersCount / totalNonCancelledOrdersOverall) * 100 : 0).toFixed(0)}% of total non-cancelled)`,
