  ...
  const attemptLoadFromDefaultSheets = useCallback(async (isInitialAttempt: boolean, isBackgroundRefresh: boolean = false) => {
    ...
    try {
      const [orderResponse, supplierResponse] = await Promise.all([
        fetch(exportOrderUrl).then(res => { if (!res.ok) throw new Error(`Order Sheet (${res.status}): ${res.statusText}`); return res.text(); }),
        fetch(exportSupplierUrl).then(res => { if (!res.ok) throw new Error(`Supplier Sheet (${res.status}): ${res.statusText}`); return res.text(); })
      ]);
      ...
      if ((error as Error).message.toLowerCase().includes('failed to fetch')) {
        detailedErrorMsg += " This can be due to network issues, incorrect sheet sharing settings (ensure 'Anyone with the link can view'), or browser extensions blocking the request. Please check your internet connection and Google Sheet sharing options.";
      } else {
        detailedErrorMsg += " Ensure sheets are public or accessible and the URLs are correct.";
      }
    ...
    return success;
  }, [loadInitialMockData, mockOrders.length, liveOrders.length, setIsLoading, setProcessingErrors, setDataMode, setDataStats, setLiveOrders, setLiveSuppliers]);
