  ...
  const handleFetchFromUrls = async () => {
    ...
    try {
      const [orderResponse, supplierResponse] = await Promise.all([
        ...
        fetch(exportSupplierUrl)
      ]);

      if (!orderResponse.ok) {
        throw new Error(`Failed to fetch Order Sheet: ${orderResponse.status} ${orderResponse.statusText}.`);
      }
      if (!supplierResponse.ok) {
        throw new Error(`Failed to fetch Supplier Sheet: ${supplierResponse.status} ${supplierResponse.statusText}.`);
      }
      ...
      if ((error as Error).message.toLowerCase().includes('failed to fetch')) {
        ...
        detailedErrorMsg += " Ensure sheets are viewable by 'Anyone with the link' and the URLs are correct.";
      }
